	...
	public static String brute_force_strStr(String haystack, String needle) {
		...
		for(int i=0;i<haystack.length();i++){
			for(int j=0,k=i;j<needle.length();j++,k++){
				if(k>=haystack.length())//试着outofindex i.e. missipi isss, test to the end 'i'
					return null;
 ...
 * 那么下一步”bacde“的hashcode=h/29+5*29^4。这是一个constant的操作，所以检测所有子串的时间复杂度只需要O(m+n-m)=O(n)，也是一个线性算法。代码如下：
 * 
